...
def create_binary_mask_from_reference(ref_img_path, height, width, threshold=128):
    """
    ref_img_path: 参考画像のパス
    height, width: 潜在空間の高さと幅
    threshold: グレースケール画像を2値化する際の閾値（0〜255）
    
    参考画像をグレースケール変換・リサイズし、閾値処理で2値化したマスクを生成する
    出力: torch.Tensor, shape (1, 1, height, width), 値は0.0または1.0
    """
    # 参考画像を読み込み、グレースケール変換
    ...
    # 閾値処理により2値化
    binary_mask_np = (ref_np < threshold).astype(np.float32)
    # torch tensorに変換（CUDAに配置）
    binary_mask = torch.tensor(binary_mask_np, device="cuda")
    # shapeを (1, 1, H, W) に変形
...
ref_img_path = "/mnt/data/IST02/characters/alleta/trace/IST02_10_164_A_0001.png"
binary_mask = create_binary_mask_from_reference(ref_img_path, latent_H, latent_W, threshold=250)
...
# 2値化マスクを用いて、元のノイズとカラーシフト済みノイズをブレンド
# foreground領域（マスクが1の部分）は元のノイズ、background領域（0の部分）はカラーシフト済みノイズを使用
z_T_key = binary_mask * original_noise + (1 - binary_mask) * shifted_noise

# 7. テキストプロンプトを指定
prompt = "1girl"  # 例として「公園で走る猫」
...
def latent_to_rgb(latent):
    ...
    # 転置して (H, W, 3) に
    latent_rgb = np.transpose(latent_rgb, (1, 2, 0))
    return latent_rgb
...
plt.tight_layout()
plt.savefig("result_image_latent_line.png")
plt.show()
